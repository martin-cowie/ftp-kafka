        ...
        let handler: Box<dyn CommandHandler<Storage, User>> = match cmd {
            ...
            Command::SiteTest => {
                ...
                return match user.as_ref() {
                    Some(u) => match storage.list(u, &cwd).await {
                        Ok(files) => {
                            let names: Vec<String> = files
                                .iter()
                                .filter(|fi| fi.metadata.is_file())
                                .filter_map(|fi| fi.path.file_name())
                                .map(|n| n.to_string_lossy().into_owned())
                                .collect();
                            if names.is_empty() {
                                Ok(Reply::new(ReplyCode::FileStatus, "No files"))
                            } else {
                                Ok(Reply::new_multiline(ReplyCode::FileStatus, names))
                            }
                        }
                        Err(_) => Ok(Reply::new(ReplyCode::LocalError, "Failed to list files")),
                    },
                    None => Ok(Reply::new(ReplyCode::NotLoggedIn, "Please log in first")),
                };
            }
